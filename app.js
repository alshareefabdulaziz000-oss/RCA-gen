function V(i){return document.getElementById(i).value.trim()}
function gH(){var v=V(“hosp”);return v===“other”?V(“hospO”):v}
function gA(){var s=document.getElementById(“act”),r=[];for(var i=0;i<s.options.length;i++)if(s.options[i].selected)r.push(s.options[i].value);return r}
function fD(d){if(!d)return””;var x=new Date(d),m=[“Jan”,“Feb”,“Mar”,“Apr”,“May”,“Jun”,“Jul”,“Aug”,“Sep”,“Oct”,“Nov”,“Dec”];return m[x.getMonth()]+” “+x.getDate()+”, “+x.getFullYear()}

function go(){
var btn=document.getElementById(“gB”);
btn.disabled=true;btn.textContent=“Working…”;
try{
var doc=buildPDF();
doc.save(“RCA_”+V(“mrn”)+”.pdf”);
btn.textContent=“Done!”;
}catch(e){
alert(“Error: “+e.message);
btn.textContent=“Error”;
}
setTimeout(function(){btn.textContent=“Generate RCA Report (PDF)”;btn.disabled=false},3000);
}

function buildPDF(){
var t=D.e[parseInt(V(“eT”))],sv=D.sev[parseInt(V(“sev”))];
var pn=V(“pN”),mrn=V(“mrn”),age=V(“age”),gen=V(“gen”),ward=V(“ward”),cc=V(“cc”);
var meds=V(“meds”),notes=V(“notes”),resp=V(“resp”)||“Medical Director”;
var stage=V(“stg”),hosp=gH(),actions=gA();
var desc=t.d.replace(/XMEDSX/g,meds);
var what=“A “+age+”-year-old “+gen.toLowerCase()+” patient (MRN: “+mrn+”) presented to “+ward+” with “+cc+”. During “+stage.toLowerCase()+”, “+desc+”.”+(notes?” “+notes:””);

var jsPDF=window.jspdf.jsPDF;
var doc=new jsPDF({unit:“mm”,format:“a4”});
var W=210,M=14,CW=W-2*M,y=M;
var bl=[3,105,161],dk=[12,74,110],gr=[26,122,58];

try{doc.addImage(“data:image/jpeg;base64,”,“JPEG”,W-M-42,y-2,42,19)}catch(x){}
doc.setFontSize(8);doc.setTextColor(40);doc.setFont(“helvetica”,“bold”);
if(hosp)doc.text(hosp,M,y+8);
y+=22;doc.setDrawColor(bl[0],bl[1],bl[2]);doc.setLineWidth(1);doc.line(M,y,W-M,y);y+=3;
doc.setFontSize(13);doc.setTextColor(bl[0],bl[1],bl[2]);
doc.text(“Root Cause Analysis Report”,W/2,y+5,{align:“center”});
doc.setFontSize(7);doc.setTextColor(80);doc.setFont(“helvetica”,“normal”);
doc.text(“Medication Safety | SPSC + MOH | Aseer Health Cluster”,W/2,y+9,{align:“center”});
y+=13;doc.setFontSize(6.5);doc.setTextColor(130);
doc.text(“RCA-”+mrn+”-”+(new Date()).getFullYear()+” | CBAHI: “+t.cb+” | “+t.spsc,W-M,y+1,{align:“right”});y+=4;

function sH(ti){if(y>255){doc.addPage();y=M;}doc.setFillColor(bl[0],bl[1],bl[2]);doc.rect(M,y,CW,6,“F”);doc.setFontSize(9);doc.setTextColor(255);doc.setFont(“helvetica”,“bold”);doc.text(ti,W/2,y+4.2,{align:“center”});y+=8;doc.setTextColor(0);}
function aR(la,va){if(y>265){doc.addPage();y=M;}doc.setFontSize(7.5);doc.setFont(“helvetica”,“bold”);doc.setTextColor(20);var ll=doc.splitTextToSize(la,45);doc.text(ll,M+2,y+3.5);doc.setFont(“helvetica”,“normal”);doc.setTextColor(40);var vl=doc.splitTextToSize(va,CW-50);doc.text(vl,M+50,y+3.5);var rh=Math.max(ll.length,vl.length)*3.2+2.5;doc.setDrawColor(200);doc.setLineWidth(0.15);doc.line(M,y+rh,W-M,y+rh);y+=rh+0.5;}
function aT(ti){if(y>258){doc.addPage();y=M;}doc.setFontSize(10);doc.setTextColor(bl[0],bl[1],bl[2]);doc.setFont(“helvetica”,“bold”);doc.text(ti,M,y+3);doc.setDrawColor(bl[0],bl[1],bl[2]);doc.setLineWidth(0.5);doc.line(M,y+4.5,W-M,y+4.5);y+=7;}

doc.autoTable({startY:y,margin:{left:M,right:M},head:[[V(“rcaT”)+” RCA”,fD(V(“dI”))+” to “+fD(V(“dC”)),stage]],body:[[“Patient: “+pn+” (”+age+“y, “+gen+”)”,“MRN: “+mrn,ward],[“Reached: “+V(“erR”),t.spsc,sv.l],[V(“rxT”),V(“own”),cc]],styles:{fontSize:7,cellPadding:2,font:“helvetica”},headStyles:{fillColor:bl,textColor:255,fontStyle:“bold”},theme:“grid”});y=doc.lastAutoTable.finalY+3;

sH(“SECTION 1: EVENT”);
aR(“Event”,what);aR(“Date/Time”,fD(V(“iD”))+”, “+(V(“iT”)||“N/A”)+“H, “+V(“sh”));aR(“Location”,ward);

sH(“SECTION 2: CAUSAL FACTORS”);
aR(“Factors”,“1. Knowledge deficit re “+t.l+” and CBAHI “+t.cb+” 2. Cognitive overload 3. Communication barriers 4. System pressure”);
aR(“Training”,t.l+” not in CME”);aR(“Competence”,“Gap in “+t.l.toLowerCase());

sH(“SECTION 3: PROCESS”);
aR(“Process”,“Failure at “+stage.toLowerCase()+”. Swiss Cheese Model needed.”);
aR(“Policy”,“CBAHI “+t.cb+” exists. Implementation gap.”);

aT(“RECOMMENDATIONS”);
doc.autoTable({startY:y,margin:{left:M,right:M},head:[[”#”,“Cat”,“Recommendation”,“Time”]],body:[[“1”,“Education”,“CME on “+t.l+”. CBAHI “+t.cb+”.”,“14d”],[“2”,“Process”,“Redesign “+stage.toLowerCase()+” workflow.”,“30d”],[“3”,“Governance”,“Monthly PT&C dashboard.”,“30d”],[“4”,“Culture”,“Non-punitive reporting.”,“60d”]],styles:{fontSize:6.5,cellPadding:2,font:“helvetica”,overflow:“linebreak”},headStyles:{fillColor:dk,textColor:255},columnStyles:{0:{cellWidth:7,halign:“center”},1:{cellWidth:14},3:{cellWidth:14,halign:“center”}},theme:“grid”});y=doc.lastAutoTable.finalY+3;

aT(“ROOT CAUSE AND ACTION PLAN”);
doc.autoTable({startY:y,margin:{left:M,right:M},head:[[“Root Cause”,“Strategies”,“Resp”,“Date”]],body:[[{content:t.r,rowSpan:3},“1. Education. 14d.”,{content:resp,rowSpan:3},{content:fD(V(“dC”)),rowSpan:3}],[“2. Workflow redesign. 30d.”],[“3. Monthly audit. 60d.”]],styles:{fontSize:6.5,cellPadding:2,font:“helvetica”,overflow:“linebreak”},headStyles:{fillColor:dk,textColor:255},columnStyles:{0:{cellWidth:42},2:{cellWidth:18,halign:“center”},3:{cellWidth:18,halign:“center”}},theme:“grid”});y=doc.lastAutoTable.finalY+3;

if(actions.length){aT(“SPSC ACTIONS”);doc.autoTable({startY:y,margin:{left:M,right:M},head:[[”#”,“Action”]],body:actions.map(function(a,i){return[(i+1)+””,a]}),styles:{fontSize:7,cellPadding:2},headStyles:{fillColor:dk,textColor:255},columnStyles:{0:{cellWidth:8,halign:“center”}},theme:“grid”});y=doc.lastAutoTable.finalY+3;}

aT(“NCC MERP SEVERITY”);
if(y>260){doc.addPage();y=M;}
doc.setFillColor(224,240,232);doc.roundedRect(M,y,CW,8,1.5,1.5,“F”);
doc.setFontSize(8.5);doc.setTextColor(gr[0],gr[1],gr[2]);doc.setFont(“helvetica”,“bold”);
doc.text(sv.l+” - “+sv.d,M+4,y+5.5);y+=11;
doc.setFontSize(7);doc.setTextColor(40);doc.setFont(“helvetica”,“normal”);
var jl=doc.splitTextToSize(“Justification: “+sv.j,CW);doc.text(jl,M,y);y+=jl.length*3+5;

aT(“CORRECTIVE ACTION PLAN”);
var ap=“Immediate (72hrs): Memo to “+resp+”. SPSC reporting. Short-term (14d): CME. Medium-term (30d): Workflow redesign. Sustained: Monthly audits, quarterly PT&C.”;
var al=doc.splitTextToSize(ap,CW);if(y+al.length*3>270){doc.addPage();y=M;}doc.text(al,M,y);y+=al.length*3+5;

if(y>190){doc.addPage();y=M;}
aT(“ISHIKAWA FISHBONE DIAGRAM”);
var cy=y+30,cx=W/2,sp=65;
doc.setDrawColor(bl[0],bl[1],bl[2]);doc.setLineWidth(0.8);doc.line(cx-sp,cy,cx+sp,cy);
doc.setFillColor(bl[0],bl[1],bl[2]);doc.triangle(cx+sp,cy,cx+sp-3,cy-2,cx+sp-3,cy+2,“F”);
doc.setFillColor(176,48,48);doc.roundedRect(cx+sp+2,cy-5,38,10,1.5,1.5,“F”);
doc.setFontSize(5.5);doc.setTextColor(255);doc.setFont(“helvetica”,“bold”);
doc.text(“MEDICATION”,cx+sp+21,cy-0.5,{align:“center”});doc.text(“ERROR”,cx+sp+21,cy+3,{align:“center”});
[{n:“MANPOWER”,it:t.fh,bx:cx-sp/2,tp:1},{n:“METHOD”,it:t.fm,bx:cx+sp/4,tp:1},{n:“MATERIAL”,it:t.ft,bx:cx-sp/2,tp:0},{n:“ENVIRONMENT”,it:t.fe,bx:cx+sp/4,tp:0}].forEach(function(c){
var ty=c.tp?cy-22:cy+22;doc.setDrawColor(bl[0],bl[1],bl[2]);doc.setLineWidth(0.5);doc.line(c.bx,cy,c.bx,ty);
var ly=c.tp?ty-5:ty+1;doc.setFillColor(bl[0],bl[1],bl[2]);doc.roundedRect(c.bx-17,ly,34,4.5,0.8,0.8,“F”);
doc.setFontSize(5.5);doc.setTextColor(255);doc.setFont(“helvetica”,“bold”);doc.text(c.n,c.bx,ly+3.2,{align:“center”});
doc.setTextColor(50);doc.setFont(“helvetica”,“normal”);doc.setFontSize(5);
c.it.forEach(function(item,j){var iy=c.tp?ty-(j*3.5+7):ty+(j*3.5+6.5);doc.setDrawColor(160);doc.setLineWidth(0.2);doc.line(c.bx,c.tp?ty-(j*3.5+4.5):ty+(j*3.5+4),c.bx+3,iy-0.5);doc.text(”- “+item,c.bx+4,iy);});
});doc.setTextColor(0);y=cy+40;

if(y>230){doc.addPage();y=M;}
aT(“SIGNATURES”);
doc.autoTable({startY:y,margin:{left:M,right:M},body:[[{content:“Prepared by:\n”+V(“s1”)+”\n”+V(“s1r”),styles:{fontStyle:“bold”}},{content:“Reviewed:\n”+V(“s2”)+”\nPharm Mgr”,styles:{fontStyle:“bold”}}],[V(“s3”)+”\nQuality”,V(“s4”)+”\nDir QM”],[{content:V(“s5”)+”\nMedical Director”,colSpan:2,styles:{halign:“center”,fontStyle:“bold”}}]],styles:{fontSize:7.5,cellPadding:3.5,font:“helvetica”},theme:“grid”});y=doc.lastAutoTable.finalY+5;
doc.setFontSize(7);doc.setTextColor(120);doc.text((hosp||””)+” | Aseer Health Cluster | Confidential”,W/2,y,{align:“center”});
return doc;
}
